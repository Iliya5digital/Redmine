...
if [ "x${OSNAME}" = "xcentos" ]; then
	...
	if [ "x$(lsb_release -s -c)" = "xwheezy" ] || [ "x$(lsb_release -s -c)" = "xtrusty" ]; then
		gem update bundler
	fi
	if [ "x$(lsb_release -s -c)" = "xbuster" ] ; then
        sed -i "/gem 'bundler'/cgem 'bundler', '>= 1.12.0', '<= 2.3.0'" Gemfile
    fi
fi
